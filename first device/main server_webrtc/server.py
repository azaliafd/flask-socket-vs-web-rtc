...
def on_join():
    ...
    sid = request.sid
    clients.append(sid)
    print(f"Client connected: {sid}, total clients: {len(clients)}")

    if len(clients) == 1:
        offerer_sid = sid
        emit('init', {'isOfferer': True}, to=sid)  # client pertama jadi offerer
    else:
        emit('init', {'isOfferer': False}, to=sid)  # client lain cuma answerer
...
def on_disconnect():
    ...
    global offerer_sid
    if offerer_sid == sid:
        offerer_sid = clients[0] if clients else None
        if offerer_sid:
            emit('init', {'isOfferer': True}, to=offerer_sid)
